...
export default function Header({ course }: { course: ContentPiece }) {
  ...
  return (
    ...
    >
      ...
      <Link href="/" className="font-bold md:ml-5 whitespace-nowrap overflow-ellipsis overflow-clip header">
        {course.name}
      </Link>
      {part && (
        <div className="md:flex flex-row items-center hidden">
          <BreadCrumbsSlash className="ml-5 mr-5" />
          <div className="font-medium text-sm text-stone-400 mx-1">{partCrumb?.name}</div>
        </div>
      )}
      <div className="md:flex flex-row items-center hidden">
