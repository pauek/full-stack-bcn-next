...
const readMetadata = async (dir: string) => {
...
export const readCourseParts = async (coursePath: string) => {
  const parts = [];
...
export const getCourse = async (): Promise<Course | null> => {
  ...
  const metadata = await readMetadata(CONTENT_ROOT);
  const course = { path: CONTENT_ROOT, ...metadata };
  const parts = await readCourseParts(course.path);
  return {
    id: course.id,
    path: course.path,
    name: course.name,
...
export const readPartSessions = async (partPath: string) => {
  const sessions = [];
...
export const getPart = async (path: string[]): Promise<Part | null> => {
  ...
  return {
    ...
    path: part.path,
    name: part.name,
    sessions: await readPartSessions(part.path),
  };
};

export const readSessionChapters = async (sessionPath: string) => {
  const chapters = [];
...
export const getSession = async (path: string[]): Promise<Session | null> => {
  ...
  return {
    ...
    path: session.path,
    name: session.name,
    chapters: await readSessionChapters(session.path),
  };
};
...
export const getAllFilePaths = async (subdir: string, extensions: string[]) => {
  ...
  for (const { id: partId } of course.parts) {
    ...
    for (const { id: sessionId } of part.sessions) {
      ...
      if (session === null) {
        console.error(
          `Session "${[partId, sessionId].join("/")}" not found!?!`
        );
        continue;
      }
      for (const { id: chapterId } of session.chapters) {
        ...
        if (chapter === null) {
          console.error(
            `Chapter "${[partId, sessionId, chapterId].join("/")}" not found!?!`
          );
          continue;
        }
        ...
        try {
          for (const ent of await utils.readDirWithFileTypes(imageDir)) {
            if (ent.isFile() && isMatch(ent.name)) {
              filePaths.push(
                join(partId, sessionId, chapterId, subdir, ent.name)
              );
            }
          }
        } catch (e) {
...
export const allSessionPaths = async () => {
  const sessionPaths = [];
